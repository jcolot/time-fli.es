...
Timeline._Band = function (timeline, bandInfo, index) {
    ...
    this._syncWithBandHandler = function (band) {
        self._onHighlightBandScroll();
    };
    this._syncWithBandOrthogonalScrollHandler = function (band) {
        self._onHighlightBandOrthogonalScroll();
    };
    ...
    jQuery(this._keyboardInput).on("keyup", function (evt) {
        self._onKeyUp(evt)
    });

    jQuery(this._keyboardInput).on("keydown", function (evt) {
        self._onKeyDown(evt)
    });
    /*
    ...
    jQuery(this._div).on("vmousedown", function (evt) {
        self._onMouseDown(evt);
    });
    jQuery(this._div).on("dblclick", function (evt) {
        self._onDblClick(evt)
    });
    jQuery(document.body).on("vmouseup", function (evt) {
        self._onMouseUp(evt)
    });
    jQuery(document.body).on("vmousemove", function (evt) {
        self._onMouseMove(evt)
    });
    jQuery(document.body).on("mouseleave", function (evt) {
    ...
    if (mouseWheel === 'zoom' || mouseWheel === 'scroll' || this._zoomSteps) {
        ...
        jQuery(this._div).mousewheel(function (evt) {
            self._onMouseScroll(evt, self._div)
        });
    }
    ...
    if (this._supportsOrthogonalScrolling) {
        ...
        jQuery(scrollbarThumb).on("mousedown", function (evt) {
            self._onScrollBarMouseDown(evt)
        });
    }
};
...
Timeline._Band.prototype._momentum = function () {
    ...
    self._dragSpeed = self._dragSpeed * friction;

    var run = function () {
        self._moveEther(Math.round(-self._dragSpeed * 20));
        if (Math.abs(self._dragSpeed * 20) > 1) {
            self._momentum();
        } else {
            self._dragSpeed = 0;
        }
...
Timeline._Band.prototype._onMouseLeave = function (evt) {
    if (this._dragging) {
        this._dragging = false;
        this._fireOnEvent("dragend");
    } else if (this._orthogonalDragging) {
...
Timeline._Band.prototype._onMouseOut = function (evt) {
    if (this._dragging) {
        this._dragging = false;
    } else if (this._orthogonalDragging) {
        this._orthogonalDragging = false;
    ...
    if (!this._lastScrollTime || ((now - this._lastScrollTime) > 50)) {
        ...
        if (this._zoomSteps || mouseWheel === 'zoom') {
            if (delta != 0) {
                ...
                this._timeline.zoom(zoomIn,
                        evt.pageX - innerFrame.getClientBoundingRect().x,
                        evt.pageY - innerFrame.getClientBoundingRect().y,
                        innerFrame);
            }
...
Timeline._Band.prototype._onDblClick = function (evt) {
    var distance = evt.pageX - this._div.getBoundingClientRect().x - (this._viewLength / 2 - this._viewOffset);
    this._autoScroll(-distance);
};

Timeline._Band.prototype._onKeyDown = function (evt) {
    if (!this._dragging) {
        switch (evt.keyCode) {
            case 27: // ESC
                break;
...
Timeline._Band.prototype._onKeyUp = function (evt) {
    if (!this._dragging) {
        this._scrollSpeed = this._originalScrollSpeed;
...
// Julien-Colot - Center events, at the minimum zoomlevel to display all events 
