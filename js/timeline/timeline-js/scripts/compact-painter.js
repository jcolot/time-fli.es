            ...
            if (filterMatcher(evt)) {
                if (!evt.isInstant() || evt.isImprecise()) {
                    ...
                    if (clustering) {
                        ...
                        for (var i = 0; i < this._bufferedEvents[this._bufferedEvents.length - 1].length; i++) {
                            sumTime += this._bufferedEvents[this._bufferedEvents.length - 1][i].getStart().getTime();
                        }

                        centerTime = sumTime / this._bufferedEvents[this._bufferedEvents.length - 1].length;
                        centerDate = new Date(centerTime);

                        var p1 = this._band.dateToPixelOffset(centerDate);
                        var p2 = this._band.dateToPixelOffset(evt.getStart());

                        if (Math.abs(p2 - p1) < 75) {
                            this._bufferedEvents[this._bufferedEvents.length - 1].push(evt);
                            previousInstantEvent = evt;
                        } else {
                            this._bufferedEvents.push([evt]);
                        ...
                        if (Math.abs(p2 - p1) < 75) {
                            this._bufferedEvents[this._bufferedEvents.length - 1].push(evt);
                            firstEventInCluster = previousInstantEvent;
                            clustering = true;
                        } else {
                            this._bufferedEvents.push([evt]);
                            previousInstantEvent = evt;
...
Timeline.CompactEventPainter.prototype.paintClusteredInstantEvents = function (events, metrics, theme, highlightIndex) {
    ...
    if (iconData.url == null) {
        ...
        iconData.iconAlign = metrics.customIconAlign;
        if (events.length <= 4) {
            iconData.className = "timeline-event-icon-cluster-" + events.length;
        } else {
            iconData.className = "timeline-event-icon-cluster-4";
        }
    }
    ...
    for (var i = 0; i < events.length; i++) {
        sumTime += events[i].getStart().getTime();
    }

    centerTime = sumTime / events.length;
    centerDate = new Date(centerTime);
...
Timeline.CompactEventPainter.prototype._paintEventIcon = function (commonData, iconData, top, left, metrics, theme) {
    ...
    if (commonData.clusterSize > 1) {
        var sumDiv = this._timeline.getDocument().createElement("div");
        ...
        sumDiv.style.padding = "1px";
        sumDiv.innerHTML = commonData.clusterSize;
        iconDiv.appendChild(sumDiv);
    }
